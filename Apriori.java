...
public class Apriori {
	   ...
	   public static List<Set<Integer>> tempSet = new ArrayList<Set<Integer>>();	  
	   ...
	   public static void Candidate_gen(FileWriter writer) throws IOException
		...
		// Create candidates of size greater than one from the current set of frequent items by the joining step.
		   
		    HashMap<Integer, int[]> tempCandidates = new HashMap<Integer, int[]>(); 
		 ...
		 // For every new candidate set generated of size 'm' check if all of its subsets
		 // of size m-1 are frequent.
	   ...
	   public static void calculateFrequentItemsets(List<Set<Integer>> tempSet, FileWriter writer) throws IOException 
	    {
		 ...
		 //Calculate the support count for the new candidates and prune those that do not have minimum support count.
	   ...
	   public static void main(String [] args) throws IOException{
			  ...
			  if(level>=k){
				  for(int [] arr: frequentOne_set){
					  ...
					  for(int i=0;i<arr.length;i++){
						  writer.write(getKeyFromValue(arr[i])+" ");
						  writer.write("("+One_set.get(arr[i]).size()+")"+"\n");
					  }
